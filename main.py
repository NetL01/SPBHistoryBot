...
# hbot.send_message(-4031826999, text=f"{datetime.now()} bot started")
...
try:
    ...
    def handle_feedback(message):
        ...
        if message.text == "Иду":
            ...
            bot.send_message(message.chat.id, "We apologize. Do you want to see the next landmark?",
                             reply_markup=get_next_keyboard())
            state = State.WAITING_FOR_FEEDBACK
            ...
            if current_landmark_index < len(sorted_landmarks):
                ...
                bot.send_message(message.chat.id, "Вы просмотрели все места в вашем радиусе.",
                                 reply_markup=types.ReplyKeyboardRemove(), parse_mode='Markdown')
                state = State.WAITING_FOR_LOCATION
        elif message.text == "Stop":
            bot.send_message(message.chat.id, "Поиск прекращён.", reply_markup=types.ReplyKeyboardRemove(),
                             parse_mode='Markdown')
            state = State.WAITING_FOR_LOCATION
